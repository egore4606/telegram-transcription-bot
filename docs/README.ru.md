...
- Gemini 3.5 Transcribe для файлов и live-расшифровки прямо в сообщении обработки.
...
Для специализированной расшифровки задайте `GEMINI_MODEL=gemini-3.5-transcribe`, а для
постепенно обновляемого текста — `GEMINI_MODEL=gemini-3.5-transcribe-live`. Резюме, TL;DR,
перевод и дополнительную очистку затем выполняет отдельная модель из `GEMINI_SUMMARY_MODEL`.
Live API ограничен десятью минутами; более длинная запись автоматически перейдёт на файловую модель.
